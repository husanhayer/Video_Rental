    ...
    class Video
    {
       ...
       of sqlcommand class that is used for database query with regarding function
        ...
        public DataTable loadData() {
            ...
            con.Open();
            command = new SqlCommand("select * from tbVideos", con);
        ...
        // this is another method with argument that is used to delete the record from the table using ID  of the video from the table .
        public void delVideo(int VideoID) {
            con = new SqlConnection(Constr);
            con.Open();
            command = new SqlCommand("select * from tbVieos where MID="+VideoID+"", con);
            int y = command.ExecuteNonQuery();
